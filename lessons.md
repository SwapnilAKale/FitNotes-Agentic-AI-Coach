...
**When a confirmation surface can grow, the scroll region must EXCLUDE the action controls.** A confirm panel sized to its content was fine for one item but, once it could hold a whole batch, grew tall enough to push the approve/reject buttons off-screen. The fix is not "scroll the panel" — that hides the confirm button too. Cap and scroll only the variable-length CONTENT region (here the preview text), keeping the action buttons as a sibling outside it so they stay pinned and clickable at any scroll position. Whenever a bounded UI region becomes unbounded, separate the part that may overflow from the controls that must always be reachable.
