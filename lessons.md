...
**A golden test pinned to a live, growing database is unsound — and the fix is synthetic by-construction tests, chosen carefully so you neither hollow the assertion nor delete the only coverage.** A batch of goldens hard-coded "most-recent" dates, counts, and computed values read from the real DB; when new rows landed they all drifted and went red, despite zero production-code change. Three repair patterns and when each is invalid: (1) **re-pin to a current read** — just defers the same break to the next data change; (2) **recompute-and-relate** (assert the answer equals an in-test recomputation) — INVALID when the recompute mirrors the production algorithm, because you check the code against itself and it passes regardless of correctness; only sound when the cross-check shares no logic with production (e.g. a differential test against a second independent implementation); (3) **freeze-the-window** — HOLLOWS a "most-recent"/"last-N-days" test, since a frozen window no longer exercises the resolution the test exists to guard. The sound fix is a **synthetic by-construction** test: a hand-built fixture where the correct answer is known by hand (counted/summed/placed by you), sharing no logic with the code under test. Two disciplines around it: before deleting a "redundant" live golden, **confirm a specific test actually covers its exact branch** — quote the covering assertion, don't assume same-function-call means same-branch; and beware **masked coverage** — a real-data test can execute a branch while being structurally unable to detect its bug (e.g. a kg-era user makes cross-frame normalization a no-op), which is zero real coverage. Synthetic regression tests are permanent — never deleted after they pass.

**State that a boundary invalidates must be cleared by the layer that SEES the boundary — driving the owner deterministically — not left to the owner or the LLM.** Staged-write state lived in an MCP subprocess and was cleared only on a successful execute; but the events that should invalidate it (page reload, a new chat turn, cancel) are only visible to the server layer, which can't touch the subprocess's memory directly. So the stale batch survived reload/cancel and the next batch stacked under it — and because a later list-append turned what had been a harmless overwrite into accumulation, a stale-state bug became a silent doubling bug (staged 7, reloaded, staged 7, confirmed → wrote 14). The fix: the boundary-observing layer (server) clears the owner (subprocess) through a deterministic control call (a discard tool invoked directly, NOT via the LLM and NOT by hoping the owner notices). Two seams enforce the invariant "committed only via explicit confirm": clear-on-entry at the start of every new turn (awaited before any new work stages) and clear-on-cancel. General lessons: (1) a single-value slot can mask a lifecycle bug that a list/accumulating slot exposes — re-audit clear-paths whenever you change a slot from overwrite to append; (2) when state and the events that should reset it live in different processes/layers, the reset must be plumbed from the layer that sees the event, deterministically.

**When a confirmation surface can grow, the scroll region must EXCLUDE the action controls — and capping the inner text alone does NOT bound the panel.** A confirm panel sized to its content was fine for one item but, once it could hold a whole batch, grew tall enough to push the approve/reject buttons off-screen. First fix attempt — `max-height + overflow-y` on the inner text `<p>` alone — FAILED live: the panel container itself was still `flex-shrink-0` (content-driven) inside an `overflow-hidden` viewport column, so heading + capped-text + buttons still overflowed and `flex-shrink-0` stopped the container yielding → buttons clipped off-screen, no scroll. The working fix bounds the CONTAINER and makes it an internal flex column: `flex flex-col max-h-[Nvh]` on the panel, heading `flex-shrink-0`, the content region `flex-1 min-h-0 overflow-y-auto` (the `min-h-0` is mandatory — a flex child won't scroll without it), and the action buttons in a `flex-shrink-0` footer outside the scroll region. Separately: render structured data (the staged batch) as a readable view — exercise cards with per-set lines and comments — not a raw `JSON.stringify` dump, with a try/catch fallback to raw text so a parse failure never blanks the panel.

**A mostly-structural golden with a live-pinned value tail: delete only the tail, keep the structure.** A broad-package test asserted drift-free structural invariants (scope label, dropped-field non-leak, aggregation level, size bound) AND, tacked on the end, a hard-coded PR value (`pr.weight == 130`) that drifted to 145 as the DB grew. Don't delete the whole test and don't re-pin the number: the structural assertions are snapshot-independent and sole coverage of broad packaging, so they stay; only the live-pinned value tail is removed, because the PR value logic — and the specific invariant the tail dressed up ("pr.comment lives on the pr object, not via full_comments") — is already proven by a synthetic by-construction test that derives pr.comment from the set list and never touches full_comments. Split a drifting golden along the structure/value seam and excise the minimum; a value already covered synthetically needs no replacement, only deletion.

**A single-value staging slot silently drops all-but-the-last in a batch; widening it to a list means updating every reader, and the dangerous one is the serializer that swallows shape errors.** A "stage one action, then confirm" slot held a single value, so N stage calls before one confirm overwrote each other and only the last was written — masked because a later verify-and-recover step papered over the loss. Converting the container (single dict → list) is the easy part; the risk is the readers. Most readers fail LOUD on the new shape (a dict-key access on a list raises), so they self-flag — but a serializer/journal that stores whatever it's handed and swallows its own exceptions (here a write-ahead-log append) fails SILENTLY at write time and only surfaces much later at replay, as an un-replayable record. When changing a container's shape, enumerate every reader, and treat the error-swallowing serializer as the load-bearing one: it must be updated to emit per-item records, not handed the whole list. (Related: the commit-then-journal ordering is a separate durability gap — a crash between the DB commit and the journal-append loop loses replay records, and per-item journaling widens that window.)

**A test that executes a branch but whose data makes the branch a no-op is coverage in name only — a sound test needs input where a bug in the branch would change the result, and you prove that by demonstration, not argument.** A normalize-then-max pre-pass had a "covering" test whose data happened to make normalization a no-op (the max was already in the target frame), so a broken conversion would not have changed the result — the line ran green regardless of correctness. The sound replacement feeds mixed-frame input where the normalized max differs from the raw max, so an absent/broken normalization produces the wrong max and the test goes red. Crucially, don't just reason that it "would" fail (that reasoning is exactly what the no-op test got wrong): temporarily introduce the bug, watch the new test go red, then revert — a demonstrated red is proof of soundness; an argued one is not.

**When the read side gains load-bearing fields, the write side must gain them in lockstep — and a "unit"-type column may be a metric-type code, not the display unit you assume.** A logging path built for weight×reps was never revisited as the read lane added comments (a 3rd load-bearing field) and cardio distance/duration; it silently wrote empty/partial rows while confirming success — the worst failure class (looks like it worked). Two specific traps: (1) the fix must mirror the read path's exact keying — comments are bound to their row by foreign key (`comment.owner_id == set._id`), so the write captures each insert's row-id and binds the comment to it (never positional/proximity matching). (2) A column named `unit` turned out to encode the METRIC TYPE (weight / duration / distance), not lbs-vs-kg — verified because rows of both display units carried the same code and the read path never read the column (display unit came from config elsewhere). Don't bind a plausibly-named param to a column without confirming what the column actually means from its real distribution. Bonus discipline: when one write is replayed elsewhere (a write-ahead log), have the live path and the replay call ONE shared writer, so they can't drift — a divergent hand-kept copy is a silent-corruption-on-replay bug waiting to happen.

**A deterministic step (commit, verify) owned by an LLM is non-deterministic by construction — move it to the layer that can do it deterministically, and remove the tool from the model's reach rather than instructing it.** A workout write gave the agent both the execute tool (call after user confirms) and the verify tool (call after execute); the agent could — and did — fire execute mid-staging (truncating a 7-exercise day to 1) and run verify against the database while the batch was still only staged (guaranteed `sets_found: 0`, misread as a failed write, re-staged, exercise doubled). No prompt wording fixes this, because the failure isn't disobedience — it's that a step with exactly one correct call site was given to a component that chooses its calls probabilistically. The structural fix: the caller that observes the user's confirm (server /confirm, CLI gate) invokes execute itself via a direct tool call, and the execute/verify tools are unexposed from the agent's schema entirely (remove the option, don't instruct against it — the agent now stages and STOPS, and there is no code path from the model to a DB write). Corollary on verify placement: verify-before-commit in ONE transaction beats write-then-check-then-delete — read the inserted rows back by their ids inside the same transaction and commit only on match; on mismatch, rollback means the dirty state never persisted, so there is nothing to clean up, no delete tool to build, and no window where a half-verified write is observable.

**Reversible-unknown testing rule: when a scenario's outcome is unknown AND the action is reversible, live-testing it is worth doing even at the risk of breaking things — the agent's real behavior under broken/unpredictable input cannot be derived from reading the prompt.** The doubled-Deadlift bug was found only by actually running a messy 7-exercise day through the live agent and watching it misfire — a code read had already "confirmed" the flow looked right, and pytest was green throughout. The economics: a reversible failed experiment costs one discard/rollback and buys ground truth about how the model actually behaves at the seams (mid-staging tool choices, misread tool results, re-stage loops); skipping the test leaves those behaviors to be discovered by the user in production. Scope the rule tightly: it applies only where the blast radius is provably bounded (staged writes behind a confirm gate, transactions that roll back, state a discard can clear) — irreversible actions (committed writes without rollback, external side effects) stay excluded and get the conservative treatment.

**A write-confirmation UI must render the actual write payload, not a re-generated summary — the two can diverge exactly when the confirmation matters most.** A confirm panel gated DB writes but displayed content built from the staging tool's ARGS (the pre-conversion input the model supplied), with the full workout otherwise visible only in the LLM's phrased answer above it. Both are re-generated representations: the args show what was *asked for*, the prose shows what the model *says* it staged — neither shows what execute will *write*. If the input→stored conversion is broken (the typed→metric weight math, a dropped comment, a mangled cardio field), an args- or prose-based preview looks perfect while the payload is wrong — the user "confirms" the one representation that hides the bug, and the confirmation is theater. The fix: the confirmation surface reads the staged slot itself — the exact structure the execute step consumes — through a deterministic server-side formatter (no LLM in the loop), so the displayed values are the stored values round-tripped back to the user's frame (typed kg/lbs recovered from the metric column, cardio as distance+duration, comments bound to their set). A useful side effect: the preview now *exercises* the recovery math, so a conversion bug becomes visible in the panel instead of being discovered in the database. Rule of thumb: whatever representation the commit step will consume is the only representation worth confirming against.

**A fallback path whose output is indistinguishable from the primary path needs positive observability — a source tag — or a live check cannot tell a working primary from a silent fallback, and a false-pass becomes structurally possible.** A confirm panel gained a primary render source (the staged slot, formatted server-side) with a defensive fallback to the old representation (the staging args). Both produce workout-shaped text, so a live run that exercised the fallback would LOOK like the primary working — and one of the two fallback triggers (the formatter returning an error payload rather than raising) logged nothing at all, making "primary fired" and "primary silently skipped" identical in both the response and the logs. The fix costs three lines: a `preview_source` field in the response ("slot" vs "args_fallback") and a stderr log on the previously-silent branch. General rule: whenever you add a defensive fallback, ask "if the primary silently stopped working tomorrow, what would tell me?" — if the answer is "nothing", the fallback isn't safety, it's camouflage. Tag the output with its source (a response field, a log line) BEFORE the live verification that depends on distinguishing the paths; a check that can't discriminate the two proves nothing.

**A live-DB-pinned assertion that cannot be soundly recomputed should be replaced by a synthetic-by-construction test, not re-pinned to a new drifting constant.** A test pinned a duration-only exercise's all-time PR to a value read from the live DB; normal use logged a longer session and the test went red with the code correct. The usual repair options both fail here: re-pinning to the new value just schedules the next break, and recompute-and-relate is UNSOUND for this case — the domain rule the test exercised (no-lock cardio PR = max distance) evaluates to zero/nothing on duration-only data, so a "correct" in-test recompute would pin to a degenerate value and stop testing the fallback branch the test existed for. When the sound recompute of a pinned value tests nothing, that's the signal the assertion never belonged on live data at all: delete it and point the branch's coverage at a synthetic fixture where the expected answer is known by construction (and verify such a test actually exists and asserts the same branch BEFORE deleting — if it doesn't, the deletion is a coverage loss, not a cleanup).

**An explicit prompt instruction can be overridden by the model when a sibling instruction a few lines away mandates the OPPOSITE phrasing for a related action — scope each phrasing rule tightly to the action it governs, and say so inline.** A staging flow''s answer rule ("never claim it was saved") sat 8 lines above a completion flow''s mandated template ("✅ … has been saved") in the same prompt section; the model blended them and reported a merely-staged write as completed. The failure isn''t a missing instruction — it''s cross-contamination between adjacent rules for similar-looking actions. Hardening that works: state the required opening positively (MUST open by saying it is staged/pending), enumerate the forbidden words ("logged", "saved", "recorded", past-tense completion), and add an explicit cross-scope note on the rule itself ("the ''has been saved'' rule below applies ONLY to X, never to Y"). And keep the blast radius in mind before editing: verify which answer surfaces are LLM-phrased vs server-templated — a phrasing fix is only safe to apply when the committed-outcome message is produced deterministically outside the model.

**An explicit user-provided boundary (a slash command) is more reliable than inferring the same intent from free text — add the explicit signal, and keep the inference as a graceful-degradation fallback rather than replacing it.** A write path that decides "is this a write?" by regex/LLM inference over free text is a standing source of false positives (a coaching question stranded in the write lane) and false negatives (a dictated workout misrouted to analysis) — every heuristic refinement just moves the boundary cases around. A user-typed prefix (`/log`) removes the ambiguous representation entirely: the user states the intent, so the boundary is deterministic and trusted, and everything downstream (routing, staging, confirmation) starts from a signal that cannot be misread. Two disciplines make this work: (1) do NOT delete the inference — an un-prefixed "log bench 100x5" must still work (graceful degradation, never silent failure); the fallback path is where a gentle, suggestive nudge toward the explicit command belongs (teach, don't warn). (2) An explicit boundary invites follow-up turns that lack it — a clarification answer ("yesterday", "3 sets of 12") carries no prefix and would misroute through the generic pipeline, so the boundary needs a tightly-scoped carry: exactly one next turn joins the flow, cleared by construction at every entry (snapshot-and-clear), with a deterministic unrelated-message rule (question/filler = abandonment) — never a sticky mode, which would just recreate the ambiguity the explicit signal removed.

---

## Semantic decision in a deterministic regex creates silent data loss

A read-only diagnosis found that a deterministic heuristic regex making a semantic boundary decision (question-shaped text = "is this a query or data?") has a false-positive direction that silently drops real data. **The pattern:** a message-parsing stage uses a regex to detect "question-shaped" strings and routes them differently from data entries. The regex test (`interrogative_word | ?` anywhere) fires on legitimate user comments that happen to contain a question mark or begin with "how/what/is/do/can" phrasing *when they aren't actually questions*. Because the regex is deterministic, the comment that fails the test is discarded silently — the user has no notice it was lost, the system has no log entry, and the grounding check (if one exists) may or may not catch it.

**Why it's a real bug, not an edge case:** comments are user data. A false-positive on the "is this a query?" question causes a false negative on "was this data preserved?" — a silent data loss from the user's perspective.

**Why the fix is architectural, not a tighter regex:** tightening a question-detection regex is whack-a-mole — "Should I go heavier?" looks like a question but is a user comment; "How am I progressing?" looks like a question and is a question. The regex can never reliably disambiguate intent when phrasing alone doesn't carry the difference. The *source* of this indeterminacy is placing a semantic decision in a deterministic rule instead of asking the system that *knows the context*.

**The fix:** move the boundary decision from the regex-evaluation stage to the stage that holds the original intent (the classification/interpretation layer). The classifier has full context — it knows whether the user is entering data or asking a question. The deterministic text-parsing regex delegates that decision *to the heuristic*, which has no context at all.

**Broader lesson:** when a deterministic rule must answer a question that requires context (intent, tone, domain knowledge), you're implementing semantics in the wrong layer. The rule will have false positives and false negatives in opposite directions — tightening against false positives creates false negatives on the semantic signal itself, and vice versa. Move the semantic decision to the layer that knows what it means.


---

## Verification against a trusted artifact is strictly easier than a second extraction — but only if the verifier's inputs are in a comparable frame

When an LLM parses free text into a structured write payload, the tempting "safety check" is a second independent extraction: parse the text again and compare the two results. That doubles the hard task and produces two probabilistic artifacts that can agree on the same mistake. The better design is a **diff against the trusted artifact**: give a verifier the original request AND the already-parsed structure side by side, and ask only "does B faithfully represent A?" — flag anything missing, extra, or attached to the wrong place. Checking a claimed correspondence is a strictly easier task than producing the structure, the same way reviewing a diff is easier than rewriting the patch: the verifier never has to resolve names, convert units, or make parsing decisions — it only has to notice a mismatch. The prompt framing matters: explicitly forbid re-derivation ("this is a DIFF, not a re-parse") or the model will drift back into doing the hard task and inherit its error modes.

Two disciplines make the diff sound:

**1. The artifacts must be in a comparable frame.** A stored payload often carries opaque internals — foreign-key ids instead of names, converted units instead of the values the user typed. A verifier diffing "log item X at 100 units" against `{ref_id: 42, canonical_value: 45.36}` is blind on exactly the fields that matter, and will either rubber-stamp or spuriously flag. Pair the raw payload with a *deterministic rendering* of it (names resolved, values converted back to the user's frame, produced by plain code — never by another LLM). The rendering is a pure function of the payload, so verifying against payload+rendering IS verifying against the payload. And if the rendering is unavailable, skip the check and mark it not-verified rather than run a blind diff — a spurious failure that discards good data is worse than an honest "unchecked".

**2. Decide the failure direction for each failure class before wiring it in.** A verdict of "mismatch" should be fail-closed (block the write, discard, ask the user to re-state — and discard *immediately*, before replying, so no stale confirmation can commit the rejected payload). But the verifier *machinery* failing (quota, timeout, unparseable verdict) should usually be fail-open when a human confirmation step still stands downstream: a transient outage must not make writing impossible, and the not-verified signal can be recorded for later stages to see. Conflating the two — treating a verifier crash like a mismatch — turns every infrastructure blip into data loss.

A residual hole to document rather than deny: a verifier can still rubber-stamp a *subtle* misattribution (a note attached to the neighboring item, values swapped between two similar entries). The diff shrinks the failure surface from "any parsing error ships silently" to "only errors the side-by-side comparison misses" — smaller, but not zero. Know which hole you accepted.


---

## Rejection messages that discard their evidence are silent dead ends — capture the deterministic artifact before cleanup

A write-path verification step checks whether a user's request matches what was staged. When the verifier returns FAIL, the staged batch is discarded immediately to close a timing leak: subsequent confirmations must find no stale batch to execute, and a restart isn't resumable from the half-confirmed state. But the FAIL message needs to tell the user *why* — which batch was wrong, what didn't match. The deterministic staged preview (the exact human-readable rendering of what would have been written) is computed *before* the discard fires, and only exists once — if the message is built *after* discard clears the MCP slot, the preview is gone and the FAIL is useless: the user gets "I misread that — re-state it" with no evidence of what they rejected. **Capture the deterministic artifact into a local variable before cleanup, not after.** The preview arrives into scope early enough at every FAIL site (server.py:606, cli.py:66, cli.py:119) — and in each case outlives the discard because nothing reassigns it. Use it at the return (lines 671, 82, 145 resp.) — if the preview happens to be empty/None (rare, but possible on a preview-read failure), fall back to a bare re-state prompt instead of crashing or silently swallowing the error.

---

## Free-text success narration is an untrusted representation — when a structural signal already knows the operation didn't happen, key off the signal

An agent's final answer text is a free-text LLM output—a claim about what happened. When a structural signal (like "no write tool was called") already knows the operation didn't occur, the user-facing outcome must key off the signal, not the narration. Conversely, detecting a divergence between signal and narration by string-matching the narration is the wrong mechanism: it polices output instead of removing the ambiguity. A related failure mode: when prior turns' state persists in conversation history as apparent pending work, the model re-acts on it, re-deriving earlier decisions as current ones. The fix is removing the duplicate representation (history vs. current request), not instructing the model to ignore history. In both cases the root cause is an ambiguous message: either "I completed the write" (false, when no tool fired) or "these exercises are still pending" (false, from prior turns). Structures that remove ambiguity—discrete signals from the code side, deterministic per-turn message scoping—are more robust than prompts that ask the model to navigate conflicting representations.

---

## Checkpoint immediately before every LLM call — and on resume, restore the verified artifact instead of re-deriving it

A pipeline with several LLM calls in sequence fails at arbitrary points (quota, transient outage). Three principles keep an interruption from losing or corrupting work:

**1. Every LLM call gets a checkpoint immediately before it, so resume re-enters at the interrupted call — never redoing a completed LLM call, never skipping an uncompleted one.** The checkpoint boundary IS the call boundary. If a stage completed, its output is in the checkpoint and resume must not pay for it again; if a stage never ran, resume must run it. The failure mode this prevents is subtle: a resumed transcript that *shows* a stage as completed (its tool results say "done") when the state that stage produced was lost with the interruption — the resumed model reads "done", skips the work, and reports success over empty state. Whatever state a completed stage produced must ride the checkpoint alongside the transcript that claims it exists, or the two will disagree on resume. Note the checkpoint writer may not be the component making the LLM call: the call site might not have access to all the state worth checkpointing (a lower layer runs the loop; an upper layer owns the flow context). In that case the lower layer checkpoints what it owns and the upper layer *enriches* the same slot on the way out of the exception — one slot, layered writers, each at its own altitude.

**2. On resume, restore the artifact — never re-derive it.** If the interrupted flow had already produced a verified artifact (an extraction that passed its verification stage), resume must restore that exact artifact, not re-run the probabilistic step that produced it. A re-derivation can silently diverge from what was verified — and then the verification that already passed is vouching for something that no longer exists. Restoring also composes with the verifier for the partial case: an artifact checkpointed mid-production restores incomplete, and the verification stage (run on resume, against the original request carried in the same checkpoint) fails it honestly instead of shipping half.

**3. A restored artifact's references must be re-validated against current state.** The world can change between checkpoint and resume — the database it points into may have been replaced, the ids it carries may no longer resolve. Validate every reference on restore; on any miss, refuse the whole restore and ask the user to redo the request, rather than confirming an artifact with dangling references. And give the restored artifact a bounded lifecycle: it must die when its flow resolves (committed, cancelled, or failed verification) — with the clear guarded so it can never destroy an unrelated checkpoint — or the restore path becomes a way to replay a committed write.

The enrichment read deserves one structural note: a helper that updates a just-written checkpoint must read the slot RAW, not through a loader that applies staleness/validity gates. A gate that can return "no checkpoint" for a file written milliseconds ago turns the enrichment into a silent no-op exactly when it matters; make the unwanted behavior structurally impossible, not just unlikely.

---

## Convert the orchestration, not the wire format — hand-building state iteration wins when the framework expects a different message shape

A framework like LangGraph provides state machines and persistence, which are genuinely valuable abstractions. The seductive move is to refactor the entire system to fit the framework's opinionated message format (tool calls as a canonical list, results appended to history, iterations driven by the framework's polling loop). Doing so trades the hand-built iteration code (often ~100 lines of explicit loop and error handling) against conforming dozens of downstream sites (tool execution, result parsing, response assembly) to a message format not native to the domain. The win is not always there: when the domain returns structured objects (dicts with typed fields) and needs to drive conditional logic (routing based on tool results, early exits, branching on state flags), fitting it into a message-appending framework just moves the conversion code, not deletes it. The better trade is to let the framework own the orchestration layer — the graph structure, the node coordination, the persistence machinery — while keeping the tool execution and state logic in their native shape. A custom tool node that calls the domain's answer() method once per iteration and handles the returned dict directly is simpler than refracting everything through messages. The nine hand-rolled seams that don't fit the framework are the cost of this choice; the benefit is keeping the domain logic coherent instead of sprinkling it across message-field assignments that must be unpacked back to their original shapes downstream.

---

## A framework's default checkpointer persists state it doesn't know can be rebuilt — exclude expensive blobs explicitly

A persistence layer's job is to remember "what was interrupted" so resume can continue. But "what was interrupted" is not always the same as "what the model needs to see." When a component holds a large computed artifact (like a package for analysis, or a processed dataset), the framework's default is to checkpoint it wholesale — byte-for-byte serialization of whatever lives in state. This is safe for small state but becomes a bloat and cost problem at scale: the framework is persisting data that can be regenerated from simpler inputs (the original request, configuration, a fresh recompute) and wastes checkpoint storage and resume time checkpointing it. The solution is explicit: declare which state fields are expensive and excluded from checkpointing via the framework's context_schema. On resume, those fields are not restored; instead, the resuming code recomputes them deterministically from the inputs that survived the checkpoint. This requires discipline: the excluded fields must be *truly* regenerable (a pure function of checkpointed state, not dependent on external non-determinism), and the resuming code must explicitly handle the empty/missing case rather than assuming the field exists. When done right, a checkpoint of a 400 KB package shrinks to 50 KB of context, session state, and message history — the verifiable, minimal facts needed to continue.

---

## A success message must be gated on a deterministic write-effect signal, never emitted as ungated model prose

An agent can narrate "successfully completed" in its final answer without any structural guarantee that the action succeeded. The agent has no inherent way to know whether its tool calls executed or were merely attempted — it can only report what the tool results said. When an action returns a "verified, written" result message, the agent should echo it. But when a tool call never happened, or when the tool returned an error, a narrated success is hallucination—the model filling in plausible-sounding prose that contradicts the facts. The guard is to key the success message off a deterministic signal from the code layer: a flag that tracks "did any write tool actually execute" or "did the verification step pass." If the flag is false, the committed answer uses a deterministic message that reflects what actually happened (e.g., "no write occurred" or "verification failed") rather than whatever free text the model generated. The text is useful for context and explanation, but the claim of completion must come from the code, not the model.

---

## Flags gate, regex disambiguates within an already-safe state — when a text check is legitimately subordinate to a structural check

A regex pattern can disambiguate within a set of cases that are already known to be safe by a structural gate. For example: a write-success claim should never ship if no write tool executed; that's decided by a boolean flag set in the code layer. Within the already-safe state (at least one write tool fired), a regex can then check whether the answer text *contains* a success claim and is thus safe to ship verbatim. The regex alone is not the gate; the flag is. The benefit of this layering is that the regex can be permissive — it doesn't need to be airtight, because a false positive (detecting a claim when none exists) is harmless when the flag already ensures the write happened. Conversely, a regex doing ALL the work (no flag backing it) becomes a precarious classification of subtle text, prone to both false positives (misidentifying a question as a claim) and false negatives (legitimate claims in an unexpected phrasing). When you find yourself tuning a regex to handle edge cases, ask whether a structural flag could gate the whole decision and leave the regex as a secondary check. The pattern is: code sets the gate (structural fact), regex operates within it (text detail).

---

## A selection window (last-k elements) can be defeated by categorical state change, not gradual drift — distinguish the state, don't widen the window

A common robustness pattern is a sliding window over recent events: "take the best of the last N days" or "average the last K values." This works when the recent events are in the same regime as the baseline — when drift is gradual. But when the recent block is categorically different in *kind* (not degree) — a deliberate shift (a deload period in strength training, a seasonal dip in sales, a tuning-focused low-volume phase), a sudden change in conditions — a larger window doesn't help. The last-7 are all deload volume; widening to last-30 doesn't "average away" the deload, it includes more deload. The fix is not to widen; it's to detect the categorical change and handle it explicitly: identify that the recent block is a distinct regime, separate it, and re-anchor the "current" measurement to the regime that preceded it. This requires two pieces: (1) a deterministic classifier that identifies the categorical boundary (stable-prior, sharp drop, length-bounded), and (2) conditional logic that applies different aggregation rules depending on the regime. A window strategy with regime-switching is more robust than a larger window in a single regime.

---

## Convert the orchestration, not the wire format — hand-building state iteration wins when the framework expects a different message shape (Session 9 — Multi-Agent LangGraph Refactor)

A framework like LangGraph provides state machines and persistence, which are genuinely valuable abstractions for orchestration (node coordination, graph topology, checkpointing). The seductive move is to refactor the entire system to fit the framework's opinionated message format (tool calls as a canonical list, results appended to message history, iterations driven by the framework's polling loop). Doing so trades the hand-built iteration code (often ~100 lines of explicit loop and error handling) against conforming dozens of downstream sites (tool execution, result parsing, response assembly) to a message format not native to the domain. The win is not always there: when the domain returns structured objects (dicts with typed fields, Gemini response objects with streaming tokens) and needs to drive conditional logic (routing based on tool results, early exits, branching on state flags), fitting it into a message-appending framework just moves the conversion code, not deletes it. The better trade is to let the framework own the orchestration layer (graph structure, node coordination, persistence machinery) while keeping the tool execution and state logic in their native shape. A custom tool node that calls the domain's answer() method once per iteration and handles the returned dict directly is simpler than refracting everything through message-field assignments that must be unpacked back to their original shapes downstream. The nine hand-rolled seams that don't fit the framework are the cost of this choice; the benefit is keeping the domain logic coherent.

---

## A framework's default checkpointer persists state it doesn't know can be rebuilt — exclude expensive blobs explicitly (Session 9 — Package Exclusion)

A persistence layer's job is to remember "what was interrupted" so resume can continue. But "what was interrupted" is not always the same as "what the model needs to see." When a component holds a large computed artifact (like a 366 KB analysis package, or a processed dataset), the framework's default is to checkpoint it wholesale — byte-for-byte serialization of whatever lives in state. This is safe for small state but becomes a bloat and cost problem at scale: the framework persists data that can be regenerated from simpler inputs (original request, configuration, deterministic recompute) and wastes checkpoint storage and resume time. The solution is explicit: declare which state fields are expensive and excluded from checkpointing via the framework's context_schema (the context outside persisted state). On resume, excluded fields are not restored; the resuming code recomputes them deterministically from inputs that survived the checkpoint. This requires discipline: excluded fields must be *truly* regenerable (pure function of checkpointed state, not dependent on non-determinism or I/O), and the resuming code must explicitly handle the empty/missing case. When done right, a checkpoint of a 400 KB package shrinks to 50 KB of context, state, and message history — the verifiable, minimal facts needed to continue.

---

## Prefer categorical signals (exact set equality) over tuned similarity margins — margins work on today's corpus, not tomorrow's (Session 9 — Resolver Rescue)

When disambiguating between similar candidates, the seductive move is a similarity score (string distance, token overlap, fuzzy matching) with a tuned threshold. The threshold gets calibrated on your corpus: "if the top candidate scores ≥0.75, auto-pick it; otherwise ask." This works today. Tomorrow, when the corpus grows or changes, the same threshold becomes fragile — a new candidate with a borderline score (0.74) gets misclassified, or an old one that relied on margin gets ambiguous. The better move is to use a categorical signal: a property that is either true or false independent of your specific data. Word-permutation matching (exact word multiset equality) is categorical: user types "dumbbell flat bench press," exercise name is "Flat Dumbbell Bench Press" — after lowercasing and ignoring word order, they're identical. No tuning, no margin, no corpus dependence. When categorical signals aren't available, accept the disambiguation (ask the user) rather than guessing based on a fragile margin. A safe "I'm not sure, which did you mean?" beats a confident wrong pick.

---

## A verification check that asserts presence cannot assert uniqueness — record the limitation rather than denying it with a stricter check (Session 9 — Display Dedup)

A verification query can check whether a fact *appears* in a result ("is this exercise mentioned?") because it's a substring search. It cannot reliably check whether a fact appears *exactly once* without dramatically higher false-failure rates, because presence and occurrence are different problems. Substring matching is the right tool for "was this cited?" (yes/no); it cannot answer "was this cited once?" (yes/no/maybe). If you make the check stricter (try to count occurrences), you create false positives on legitimate duplicates (the same weight×reps appearing in two exercises' session displays, a warmup and working set with identical numbers). Rather than making the check stricter and living with false failures, document the gap: "each citation string must appear somewhere in the output; we do not verify it appears exactly once." The gap is real and worth recording; denying it with a strict check and accepting edge-case false failures is worse than honest "we check this, not that."

---

## The executor catches wrong premises in the plan — details hidden in the plan's earlier stages surface at implementation (Session 9 — Resolver LIMIT Bug)

A multi-layer pipeline (plan → execute) can fail when assumptions baked into an earlier stage are incomplete. Tier 3 (LIKE matching) generates a candidate pool by applying a limit early and a filter later: "SELECT * FROM exercises LIMIT 8 ORDER BY name, then check if ≥ 2 words." By the time the filter runs, the true match ("Flat Dumbbell Bench Press," alphabetically late) is already dropped by the LIMIT-8. The plan assumed "the top N alphabetically are a superset of the relevant candidates" — a true assumption if the filter is name-based, but false when the filter is multi-word. The executor caught this by failing on real input, not by reading the plan. A query-building plan written against incomplete reasoning needs real-world verification. The lesson: the executor doesn't re-verify the plan's assumptions; it just exposes failures when they surface. If the plan is wrong, a test against the real data will show it. If the plan is right, the executor confirms it.

---

## Stored memory outlives its context — anchor relative statements at the single write chokepoint (Session 10 — Memory Relative-Time Fix)

A long-term memory store that feeds facts back into future prompts has a hidden time bug: a fact phrased relative to its write date ("progress stalled over the last 70 days") is true the day it is written and drifts false every day after, because the readers emit content only — the fact's creation timestamp never reaches the model. The fix has two layers with very different trust levels. The instructional layer (tell the extracting LLM today's date and require absolute ranges) improves the common case but cannot be trusted alone — especially when the prompt's own few-shot example *models the bad habit* ("for the past 3 months"), which is worth auditing for in any extraction prompt. The load-bearing layer is deterministic: at the single write chokepoint every path funnels through, detect relative-time phrasing with a deliberately broad regex and append a fixed anchor ("as of YYYY-MM-DD"). A relative statement plus a fixed date is disambiguous forever. The guard's false positives are free (an extra anchor never corrupts meaning), so the regex can bias broad — a rare case where over-matching is the correct calibration. One subtlety: dedup must compare stamp-stripped content, or the same fact re-extracted on a different day differs only by its stamp and stores twice.

---

## A dev script named like a test is a test — collection executes module top-level code (Session 10 — Pytest Collection Hygiene)

Any file matching `test_*.py` reachable from pytest's rootdir gets imported at collection time, and import runs all module top-level code. A manual dev script named `test_memory.py` sitting in a `scripts/` folder — 100 lines of top-level statements including a live LLM API call — turned every bare `pytest` run into a quota-burning multi-minute hang that errored the collection. Worse, its repeated manual runs had silently polluted the production vector store with hundreds of orphaned test facts (254 of 260 entries), because it wrote through the real embedding path. Two cheap invariants prevent the class: (1) an explicit `testpaths` in pytest config so collection never wanders into script folders; (2) dev scripts that exercise storage must point every store they touch (not just the obvious one) at test paths. The pollution was found only because a fix's verification step queried the store and got back contradictory duplicates — reading your production stores occasionally is itself a check.

---

## When every strategy for reinterpreting data is dangerous, refuse the input at the boundary and name the correction (Session 10 — Native-Unit Guard)

A user typed a weight in the "wrong" unit for an exercise, and three fix strategies were evaluated in sequence. (1) Entity-level config toggle ("this exercise is kg from date X"): rejected because the primary data source is a re-uploadable app backup — a config change made outside the app contradicts every row the app writes afterward. (2) Per-record marker (annotate the individual set with its unit): survives re-upload and round-trips correctly at the single-record level, but every AGGREGATE over that exercise — volume, PR comparison, session labels, projections — now needs mixed-frame awareness for one odd record among hundreds. The set-level fix was provably correct and still wrong at the system level: correctness of a record is not correctness of the aggregates over it. (3) The chosen fix: a deterministic guard at the write boundary that refuses the non-native unit outright, states the exercise's actual unit, and includes the tool-computed conversion so the user can simply confirm the converted number. This matches the source app's own constraint (its UI cannot express the mismatch at all) and two standing principles: remove the wrong option rather than resist it, and a silent wrong write is the worst outcome. The general lesson: when both "adapt the config" and "annotate the data" require the rest of the system to get smarter, the cheapest correct system is one where the invalid input cannot enter — refusal with a precise, actionable message is a feature, not a limitation. And an implementation detail worth keeping: the refusal's conversion arithmetic belongs in deterministic tool code, never delegated to the LLM relaying the message.

---

## A gate armed at attempt-time will lie about outcomes — arm gates on verified state (Session 11 — Ghost Confirmation Panel)

A human-approval gate (the staged-write confirmation panel) was armed inside a pre-call hook: "the logging tool is being CALLED, therefore a batch will exist, therefore show the panel." The hook fires before the tool runs, so it structurally cannot see the outcome — and the moment the tool gained a refusal path (the native-unit guard, any needs_clarification return), the invariant broke: the panel appeared for a batch that did not exist, rendered a raw-args fallback as if it were real, and swallowed the agent's actual question to the user. Worse, the false signal was symmetric: users saw panels for nothing and chat-text "success" for nothing, and could no longer tell either apart from the real thing. The fix is one move: the gate reads the staged slot — the exact state the execute would consume — and a PROVEN-empty slot means "this was never a confirmation event," falling through to the agent's answer. Two calibration details matter. First, the epistemic split: proven-empty (successful read, empty list) suppresses; UNKNOWN (read failed) fails toward the panel, because executing an empty slot is a harmless no-op error while suppressing a real batch loses a write. Second, the same seam must be fixed at every interface at once (web panel and CLI finalize both armed at attempt-time). The general form: any UI promise about an action's outcome must be derived from the action's recorded result, never from the fact that the action was attempted — attempt-time arming is a time bomb that detonates the day the action learns to say no.

---
## Shared mutable files need isolation at the fixture root, not per-test discipline (Session 12 — WAL Test Pollution)

The WAL journals every confirmed DB write so an uploaded backup can be replayed onto. It accumulated 1,025 records of which 986 were pending — and an audit showed the 986 collapsed to 33 unique batch signatures repeated 3–58 times each, timestamped in clusters exactly matching pytest-suite runs. Zero were genuine user workouts. The root cause was a hygiene asymmetry: the shared conftest isolated the graph-checkpoint path for every test, but the WAL path was left to per-test discipline — a handful of tests remembered to patch it, a dozen others reached the real execute path and journaled into the real file, invisibly, across weeks of green suite runs. Had a backup upload happened first, replay would have injected ~979 junk workouts into the user's real database. Three lessons compound here. First, any file a test *can* mutate must be isolated by an autouse fixture at the conftest root — per-test patching is a memory test the suite will eventually fail. Second, add a belt-and-suspenders guard in the resource module itself keyed to a structural fact (PYTEST_CURRENT_TEST in the environment, inherited by subprocesses) that redirects rather than drops, so even unisolated code paths cannot hit the real file. Third, the money proof for this class of fix is byte-equality: hash the real file, run the whole suite, hash again — identical or the guard is fiction.

---
## The same isolation disease recurs per shared resource — audit every mutable file a test can reach, not just the one that bit you (Session 13 — DB Pollution + User Replay Controls)

The WAL pollution had a sibling: the same unguarded tests had also written 77 rows and 11 comments directly into the user's real database — the DB's "newest workout" was a synthetic test pyramid, and one test had even pinned those junk rows as expected values, meaning the suite would have FAILED against a clean database. Fixing the WAL seam and stopping there would have left the worse half standing. When one shared mutable resource turns out to be unprotected, enumerate every other file the same code paths can reach (DB, settings, checkpoints) and isolate them all at the conftest root in the same pass; for a database whose tests pin real values, isolate with a per-test COPY of the real file so realism reads survive while stray writes divert. Forensics note: insertion-order ids beat content heuristics — every phone-native row sat below one _id boundary, making the junk band provable row-by-row against archived journal signatures instead of guessed from "suspicious" values. And when exposing an internal mechanism to the user (replay on/off, wipe), name it by its effect in their vocabulary ("apply chat-logged workouts to uploads"), never by the implementation's name.

---

## Enforce the positive invariant; never blocklist the values that happened to be wrong (Session 14 — Recency Guard)

An agent asked for "my most recent session" occasionally answered with a real but non-latest date — a salient one it had seen earlier in the package. The tempting fix is a blocklist: notice the dates that keep coming back wrong and exclude them. That fix is exactly as good as the sample that produced it, and it silently stops working the day the data changes. The correct shape is to state the invariant the answer must satisfy and enforce it deterministically: for a recency claim, `date == latest_session_date`, checked post-draft against the package, with the correction applied in both ISO and human date formats. The general rule: an invariant is a property of the *predicate* being answered, not a property of the wrong answers you have collected. Anchor the guard on the predicate, and it holds for data you have never seen. A corollary worth stating: this class of guard belongs *after* the draft and *outside* the model, because you are checking a claim, and a model asked to check its own claim is being asked the same question twice.

---

## An answer format forced on every turn is a bug for the turns that did not ask for it (Session 14 — Display Routing)

A session-display block was attached to every answer that had display data available, on the reasoning that more context cannot hurt. It can. An analytical question — "is my squat progressing?" — was getting a whole-day set dump stapled to a trend answer, which buried the actual answer and, worse, invited the model to narrate the dump instead of the trend. The instinct is to fix it downstream with a prose rule ("don't list sets unless asked"), which is a rule the model will follow most of the time. The structural fix is upstream: make display an explicit *intent* the classifier emits, and attach the display block only for display-shaped questions. The general form: when an output section is optional, the decision to include it is a routing decision with its own signal, not a default with an exception list. And when the same content can legitimately arrive twice, dedupe by whitespace-tolerant containment and append only what is missing — a dedup that replaces rather than appends will eventually drop the half you needed.

---

## Build the producer before the consumer, and ship it inert (Session 14 — Decomposition Stage 1)

Splitting a compound message into independently routed requests is a change with a large blast radius: the classifier, the state schema, the executor, the merge layer and every routing test. Staged as one change it is unreviewable, and a regression anywhere looks like a regression everywhere. The staging that worked was to ship the *producer* first and leave it **inert**: the classifier began emitting a per-chunk requests array that nothing consumed, alongside the unchanged flat fields that still drove every decision. That bought two things a design document cannot. First, real field data — a divergence logger recorded every disagreement between the flat verdict and the chunked one across live traffic, which is what the consumer stage was then designed against. Second, a genuinely zero-risk deploy: no behaviour could change, because no code read the new field. Two implementation details generalize. The sanitizer treats the new array as all-or-nothing — any malformed entry drops the whole array rather than admitting a half-parsed structure — because a partially valid plan is worse than no plan. And raising the output token ceiling was not optional: a truncated JSON response degrades to the parse-failure default, which for a write intent means silently not writing.

---

## A one-way door needs a strength, not a boolean (Session 15 — Write Signal, and Session 18 — Routing)

A regex pre-guard that force-routes a message to the write path is protecting against an unrecoverable outcome — a lost write — so the instinct is to make it fire eagerly. But eager firing has its own unrecoverable outcome in the other direction: a coaching question ("should I add weight to my squat?") force-routed into a write agent strands with no answer and no recourse. Both directions are bad and no regex separates them, because "I did chest and triceps today" (record this) and "I did shrugs today and my grip gave out" (explain this) are the same shape. The resolution is to stop treating the signal as a boolean verdict and give it two strengths. An explicit write verb **binds** — the user named the action, so it overrules even a successful classification. Verb-less narration only **hints** — it spends the classify call and stands in when that call *fails*, but defers to a successful one. The classifier, which can read intent, decides; the regex's only remaining job is to guarantee a write is never lost to an *error*. Generalized: when a cheap heuristic guards an expensive-to-reverse action, do not ask it for the verdict. Ask it what it is confident about, and let it degrade to a fallback for the failure case only.

---

## A gate armed from a structurally-always-false signal fires every time (Session 15 — Carry Mis-arming)

A "continue this flow next turn" carry flag was armed on a condition meaning "the agent staged something but no execute tool ran". That was a fair reading when the agent executed its own writes. It stopped being one the day the *host* took over execution: under the new design the agent never runs execute for a workout, so the condition is structurally false on every successful staged write, and the carry armed every single time — hijacking the user's next message across the board. The bug is not that the condition was wrong; it is that the condition was a *proxy*, and the thing it proxied for had moved. Two lessons. First, when responsibility for an action moves between components, grep every flag that was reading the old component's behaviour as evidence — a proxy silently inverts rather than erroring. Second, add the positive condition rather than patching the negative one: "anything was staged this turn" is a fact about this turn, where "no execute ran" is a fact about someone else's turn. Defense in depth earned its place here — the confirm, cancel and verify-failure paths all clear the carry explicitly, because a flag that has already proven it can mis-arm should have more than one place that can unstick it.

---

## Resolve ambiguity before execution, never inside it (Session 15 — Structured Disambiguation Panel)

A compound message — "is my squat progressing, and log bench press 100x5 today" — lost the write entirely and hijacked the following turn. Two roots, one shape. The analytical chunk hit an ambiguous name and armed the "ask the user" slot from *its own* parameters, so resuming dropped the sibling write. And the write chunk's ambiguity was **agent-internal**: the model asked "which bench press?" in prose, which is invisible to the orchestrator — nothing was staged, so the coordinator saw a turn that had produced no write and armed a carry that then ate the next message. The fix is to hoist resolution above execution: resolve *every* name across *every* chunk before any chunk runs, collect all the ambiguous ones into a single structured panel, and hold the whole turn until it is answered. Three details matter. Resolution is permissive for reads and **strict for writes** — never auto-pick a name that will be written. The panel is structured data (candidate buttons plus a free-text escape), not prose, precisely so the orchestrator can see the question and the answer. And when the same seam exists on two interfaces, both must route through one shared tail rather than two copies that will diverge. The general principle: an ambiguity the orchestrator cannot see is an ambiguity it cannot recover from — surface it as state, never as text inside an answer.

---

## Self-contained means self-contained — shared history leaks into a decomposed part (Session 16 — Sibling Confabulation)

A compound turn was split into an analytical chunk and a write chunk, and the analytical answer volunteered a disclaimer about the *sibling's* exercise: "I only have Sumo Squats data, please update your Bench Press data." Nothing in the squat chunk mentioned the bench. The leak was shared conversation history: each chunk carries a self-contained restatement of its own intent, but the package builder was still dumping the whole session's history into the analysis call, and a prior turn naming the bench was enough to make a squat-only draft editorialize about it. The lesson is that "self-contained" is a property of *every* input to the call, not just the one labelled as the intent. If a chunk is a full restatement, it must not inherit context — a decomposed chunk now sets an explicit suppression flag and the builder gates history off on it. The prompt-level companion fix generalizes beyond decomposition: conversation context is framed as *background only*, with an explicit rule against noting missing data for an entity absent from the current question. Models treat any provided context as fair game to comment on; if you do not want it in the answer, do not put it in the call.

---

## De-flake a rolling-window test by widening the population, never the window (Session 16 — Warmup Invariant)

An invariant test required a specific exercise to have a session with no warmup sets inside a today-relative 90-day window. That precondition was true when written and quietly went empty as the window advanced and the user's habits changed — producing a red test against provably correct product code. The two tempting fixes are both wrong: widening the window to 365 days only postpones the same failure and slows the suite, and pinning to a fixed date range means the test stops exercising current data. The right move is to stop depending on any single entity: build the fixture broadly and scan the entire package for *every* session matching the precondition, which turned hundreds up instead of one. Two riders. State the invariant over the population, and assert that the population is non-empty — otherwise "no violations found" silently becomes "nothing was checked". And scope the assertion to rows that still carry the field being checked, because a package that trims by scope will legitimately drop it from some rows, and reading a trimmed row as a violation is a bug in the test, not the code.

---

## A closed reference set stays closed by mechanism, not by discipline (Session 17 — Muscle Taxonomy)

Some reference data is universal and must never grow at runtime: every human has the same 39 muscles, so a pipeline that can *add* one has a defect, not a feature. Stating that in a comment is worth nothing, because the code paths that would add one are the same paths that legitimately add everything else. Three independent mechanisms enforce it here: a frozen snapshot test pinning all 39 rows so any addition turns the suite red; the file excluded from the writable-files set with an assertion at the single place bytes are appended; and the name resolver rejecting an unknown name rather than creating it. Any one of the three could be bypassed by a determined refactor; all three failing silently is not plausible. The reason to spend three mechanisms on one rule is blast radius — the taxonomy is universal, so one bad automatic addition propagates to every future user of the system, and there is no per-user place where it would look wrong. Where the same store *is* allowed to grow (the exercise-to-muscle edges), growth is gated on **categorical** evidence only: normalized-exact, or a sole exact word permutation. Fuzzy similarity is deliberately not a tier, because "close enough" is precisely how hundreds of sets get attributed to the wrong muscle with nobody noticing. A real typo goes to human review, and unsure means the data stays excluded.

---

## Adding a third case exposes every else-branch that assumed there were two (Session 17 — The Limiting Role)

Exercise-to-muscle edges had two roles, primary and secondary, and the routing was written as "target = primary_ids if role is primary else secondary_ids". That line is correct and unremarkable for exactly as long as two roles exist. Introducing a third — **limiting**, for a muscle that is *held* rather than trained — would have silently routed it into secondary and counted held sets as training volume: the precise outcome the new role was created to prevent, arriving as a side effect of adding it. The general hazard is that a binary dispatch written as if/else encodes "not X" where it means "Y", and the two diverge the moment a third case appears, without a type error, a test failure or a warning. The fix is to make routing explicit on all branches and to make an *unrecognized* value count nowhere rather than defaulting into the last branch — a role the code does not understand should contribute nothing, not contribute to whichever bucket happened to be the else. Reverting that one line turned four tests red, which is the standard the change should be held to. There is a domain lesson underneath, too: the audit question had been "does this muscle do enough work here to matter?", which conflates two different claims. Grip caps how much you can shrug, but shrugs do not train the grip. When neither available answer is right, the question is wrong — and the fix is a new distinction, not a better threshold.

---

## Two implementations of one rollup means two sets of numbers (Session 17 — Viewer Rollup)

The 3D viewer needed per-muscle set counts, and the counts were already computed server-side for the analysis package. Reimplementing the aggregation in JavaScript for the client looked cheap and immediately diverged: listing per-edge overstated one region by 588 primary sets, credited another with 312 secondary sets where the truth is zero, and placed the same lift in both columns. None of that was a hard error — every number rendered, and every number was wrong in a way only a side-by-side comparison would reveal. The rule is that a derived figure has exactly one implementation, and every surface reads it; a second implementation is not a copy, it is a fork that will drift on the first change to either side. The subtler trap is that the divergence here came from a *modelling* difference rather than an arithmetic one — the server applies role precedence (a muscle genuinely trained by a lift is not demoted because another path merely leans on it), and the client's naive per-edge listing had no notion of precedence at all. Business rules embedded in an aggregation do not survive being re-expressed; ship the aggregation, not the recipe.

---

## Geometry cannot be verified by reading it — build the rasteriser (Session 17 — 3D Viewer)

Three defects shipped in a row on a 3D viewer: a spinner that hung, a canvas blown to twice its width, and a figure that rendered as a stick. All three were **blind** failures rather than thinking failures — the code read as correct, and there was no way to see the output without a browser and a human. The fix that ended the streak was not more careful review; it was a small script that rasterises the *same* source geometry to an ASCII silhouette from three angles, giving a text artifact that a test can assert on and a developer can eyeball in a terminal. Generalized: for any output whose correctness is *perceptual* — geometry, layout, rendered documents, charts — a reviewer reading the source is not a feedback loop, and confidence gained that way is unearned. Build the cheapest possible renderer into the toolchain and look at it. A related structural choice pays off for the same reason: muscle positions are computed as volume centroids rather than typed into a table, which makes "a muscle floating outside the body" *unrepresentable* instead of merely unlikely — the hand-typed table it replaced had the arms 2.2x wider than any plausible torso. Prefer a representation in which the bad state cannot be expressed over a check that catches it.

---

## An unsigned native dependency is a single point of failure — remove the need, don't disable the protection (Session 17 — uuid_utils Shim)

Windows Smart App Control blocked a transitive dependency's unsigned binary extension, and the failure cascaded: the import took down the LLM framework, which took down the orchestration layer, which took down the entire analytical pipeline and every test that imported it. Two of the three obvious responses are bad. Reinstalling fetches the identical unsigned binary. Disabling the OS protection is a system-wide, irreversible security downgrade to accommodate one file — trading a real security property for a build convenience. The third response is to remove the *need* for the binary: the whole dependency chain used exactly one function from it, a timestamp concatenated with random bits, specified in an RFC and requiring no native code. A pure-Python shim that tries the real extension **first** and falls back only when the OS blocks it keeps unaffected machines byte-identical in behaviour, which is what makes the shim safe to leave in place and safe to delete later. Two riders. Version-control the shim at a path that precedes site-packages, or a routine reinstall silently reintroduces the failure. And an *optional* plugin must never be able to take the whole test suite down at collection time — the same dependency was being pulled in by a tracing plugin nothing in the project used, which was disabled outright. One bug found while building the fallback is worth its own sentence: a monotonicity guard was overriding *explicit* timestamps, so asking for a 2023 identifier silently returned 2026. An explicit argument is an instruction, and a guard for the default case must not apply to it.

---

## A rule written in two languages will drift, and neither side can see it (Session 18 — Routing De-duplication)

The lane decision was expressed twice: in English inside the classifier's system prompt, and in Python inside the regex gates. Neither copy referenced the other, so they drifted into disagreement that was invisible from either side — the prompt declared one phrasing analytical while a regex claimed the same phrasing for the write path and won, making the prompt's rule *unreachable* for that input. Two independent "is this a question?" detectors disagreed on four of seven ordinary coaching questions; the one guarding the write path did not consider "am I overtraining" a question. Duplication in prose is worse than duplication in code, because there is no compiler, no import graph and no rename tool — nothing mechanical will ever tell you the copies exist. Three moves fixed it. Collapse each concept to one implementation (one question test, one quantity test, one mixed-lane test — previously two, two and three copies). Restructure the prompt so each rule is stated exactly once; the analytical lane had been defined four times, and the *first* definition excluded the other three. And write the ordering down as a document, because the precedence had previously existed only as statement order inside one function and had to be re-derived by every reader — which is how it drifted in the first place. The verification technique generalizes: write the gate table so it passes against **unmodified** code first, then confirm the fix turns red exactly the rows it targets and nothing else.

---

## A prompt that describes a system that no longer exists is worse than no prompt (Session 19 — Operational Prompt)

An agent's system prompt still instructed it to check whether a date "exists in the database" before inferring a year — but every read tool had been stripped from that agent, so it was being told to branch on a fact it structurally could not obtain. The worked example was worse than useless: it tried the same date twice and pointed at a year that has not happened. Stale prompt text does not fail loudly; the model does its best with an impossible instruction and produces confident nonsense. Three related findings came out of the same audit and generalize. A tool was exposed to the agent, documented nowhere in the prompt, and absent from the confirmation gate — an undocumented capability is an ungoverned one, and this one could silently destroy a staged workout. A constant named WRITE_TOOLS was doing two jobs at once, "needs the user's approval" and "can change stored data", which worked only while those happened to describe the same set; splitting it into two named sets was required before a discard could be confirmable without being claimable as a save. And one tool's semantics were stated **twice, in opposite directions** — the verify group said false meant success, the actions section said the reverse — so the prompt was instructing the model to report a successful delete as a failure. Pin such a statement against the server's own tool description rather than restating it by hand. When two agents share a behaviour, compose both from one definition and assert them byte-identical; the copies here had drifted such that a performance complaint drew a medical redirect from one agent and not the other.

---

## An actor told nothing about an action taken on its behalf will reason correctly from a false premise (Session 19 — Host-Executed Writes)

A workout was staged by the agent, confirmed by the user, and committed by the *host* — deliberately, so the agent cannot bypass the confirmation gate. But the commit happens outside any agent turn, so nothing updated the agent's history and its last known state remained "staged, awaiting confirmation". When the user then said "delete that set", reaching for the discard-staged-writes tool was **correct reasoning from a false premise**, and the user was told the workout had been discarded while the row sat untouched in the database. Data staying while the user is told it is gone is the worse direction of that failure: they stop looking for it. Three layers had to fail, and each needed its own fix. The premise: the host now records its own verified outcome back into the agent's history — its own outcome, not a re-description, since the host executes the agent's slot unmodified. The tool: discard answered identically whether or not anything was staged, so it now reports whether there was a pending batch and, when empty, says outright that it cannot remove saved data and names the tool that can — the result *corrects* the mistake rather than confirming it. The claim gate: an audit found 11 of 17 plausible completion claims undetected, and on goal and set-edit turns 12 of 13, because the detector was scoped by logging-flow rather than by the structural fact that a write tool was called this turn. Worth recording honestly: the root-cause fix did **not** change tool selection — the agent still reaches for discard — and it was the second layer that corrected the outcome. Defence in depth earned its place precisely because the root-cause fix did not land the way it was predicted to.

---

## A tool that answers identically whether or not it did anything cannot be trusted by its caller (Session 19 — Empty Discard)

Stated on its own because it is the most portable form of the above. An operation returned "discarded: true" on an empty slot — technically defensible, since after the call nothing is staged either way, and catastrophic in practice, because the caller relays that as "I removed it" to a user who asked about data the operation never had the power to touch. A tool's return value is consumed by something that cannot see what the tool saw; a value that is constant across the "did work" and "did nothing" cases has thrown away the only fact the caller needed. Every operation should report whether it *found* what it acted on, separately from whether it *succeeded*. The same defect in a different costume: a delete that matches zero rows raises no exception, so a success flag derived from "no exception was raised" means only that the SQL was valid — a claim about syntax being relayed to the user as a claim about their data.

---

## A citation gate proves which field was read, never what the sentence means (Session 20 — Limiting Guard, in flight)

Answers are grounded by making the model tag every factual claim with the exact field it came from, then resolving those tags deterministically against the data. It is a strong mechanism and it has a precise limit. Live, a draft cited the limiting role correctly — one clean tag, resolving to the right field — and still wrote "your Lat Pulldowns *do train* your biceps", which is the exact opposite of what limiting means. The citation was not wrong; it proved the model read the right field, and said nothing about whether the sentence built on it was true. Semantic invariants therefore need enforcing the same way recency does: a deterministic post-draft guard that reads the structured truth, detects the contradicting assertion, and repairs the sentence. Two design notes carry over. The *graph decides, the regex only locates* — pattern matching finds where a claim about a muscle appears; whether that claim is false comes from the edge data, and the entity vocabulary is read from the store rather than matched as open text. And guards of this kind should be written against a synthetic fixture that exercises both directions explicitly, because a guard that only ever sees violations is untested against the far more common case where it must stay silent.

---

## Two filters that each narrow can be empty by construction (Session 20 — Scope Intersection, in flight)

A query carried two scope parameters — exercise names and muscle groups — and both were applied as row filters, ANDed. "Do my lat pulldowns train my biceps?" yields exercise Lat Pulldown and muscle group Biceps, and the source app files Lat Pulldown under **Back**, so the conjunction was empty *for every window, every user, forever*. The pipeline fell back to broad scope and confidently reported an exercise with 366 logged sets as absent from the history. The sharp edge is that the single-category-per-exercise limitation was the exact thing the muscle ontology had been built to work around — and the cross-category question was being killed by that category before the ontology was ever consulted. Two lessons. Parameters extracted from one sentence are not necessarily all *restrictions*: when exercise names are present they **are** the scope, and a muscle group named in the same sentence is the **subject** of the question. Before ANDing two extracted filters, ask whether they are two narrowings or a narrowing and a topic. And pin the fix as an invariant over many groups rather than as the one instance that was reported — the reported case is one sample of a class, and here the class was every exercise crossed with every group it is not filed under.

---

## A model that knows the data is missing can decline; one that does not know will invent (Session 20 — Unresolved Names, in flight)

When a named exercise resolved to no rows, the field recording that was **removed** from the package before the model saw it, and a disclaimer was string-prepended to the finished answer instead. The result was the worst of both: the model answered a question about a named exercise with no idea it had no data for it, filled the gap from aggregate counts belonging to *other* lifts, and produced a confident claim that directly contradicted the note stapled above it. The user then had to adjudicate between two sentences the system had emitted in the same breath. Withholding the absence does not prevent the model from claiming something — it only removes its ability to notice it should not. Leave the gap **in** the data, explicitly marked, and the model's no-claims instruction becomes enforceable rather than decorative. The general form: a post-hoc annotation cannot correct a generation that has already happened; it can only argue with it. Any fact the answer must respect has to be an input to the call, not a wrapper around its output.
